...
public final class Context {
    ...
    public void run() {
        ...
        while (true) {
            ...
            // Pentru fiecare operatie trackable am adaugat in staging mesajul corespunzator.
            if (!wasError && this.isTrackable(operation)) {
                // TODO If the operation is trackable, vcs should track it
                String message = null;
                if (operationString.toLowerCase().startsWith("touch")) {
                    message = "Created file " + operation.getOperationArgs().get(1);
                }
                if (operationString.toLowerCase().startsWith("mkdir")) {
                    message = "Created directory " + operation.getOperationArgs().get(1);
                }
                if (operationString.toLowerCase().startsWith("writetofile")) {
                    message = "Added \"" + operation.getOperationArgs().get(1) + "\" to file "
                            + operation.getOperationArgs().get(0);
                }
                if (operationString.toLowerCase().startsWith("rm")) {
                    message = "Removed " + operation.getOperationArgs().get(1);
                }
                if (operationString.toLowerCase().startsWith("cd")) {
                    message = "Changed directory to " + operation.getOperationArgs().get(0);
                }
                if (message != null) {
                    if (vcs.getStaging() == null) {
                        List<String> staging = new ArrayList<>();
                        staging.add(message);
                        vcs.setStaging(staging);
                    } else {
                        vcs.getStaging().add(message);
                    }
                }
            }
        }
